...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    aggregations = delivered.groupby('order_id')[['freight_value', 'product_weight_g']].agg('sum')
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    order_purchase_ammount_per_date = filtered_dates.groupby(filtered_dates['order_purchase_timestamp'].dt.date).size().reset_index(name = 'count')
    result_df = pd.DataFrame(
        {
            'order_count': order_purchase_ammount_per_date["count"],
